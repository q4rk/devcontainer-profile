...
ensure_root() {
    if [[ $(id -u) -eq 0 ]]; then
        ...
        if command -v sudo >/dev/null 2>&1; then
            ...
            # and -E to preserve the rest of the environment (including PATH)
            sudo -n -E -H "$@"
        else
            "$@"
        fi
...
init_workspace() {
    mkdir -p "${STATE_DIR}" "${WORKSPACE}/tmp" || return 1
    if [[ "$(stat -c '%U' "${WORKSPACE}" 2>/dev/null)" != "$(id -un)" ]]; then
        ensure_root chown -R "$(id -un):$(id -un)" "${WORKSPACE}" >/dev/null 2>&1 || true
    fi
...
reload_path() {
    ...
    local critical_paths=("/usr/games" "${HOME}/.local/bin" "${HOME}/go/bin" "${HOME}/.cargo/bin" "/usr/local/cargo/bin" "/usr/local/go/bin")
...
main() {
    ...
    info "=== STARTING DEV CONTAINER PROFILE ENGINE ==="
    ...
    if [[ "$(stat -c '%U' "${VOLUME_CONFIG_DIR}" 2>/dev/null)" != "$(id -un)" ]]; then
        ensure_root chown "$(id -un):$(id -un)" "${VOLUME_CONFIG_DIR}" >/dev/null 2>&1 || true
    fi

    # only ingest if the volume is currently empty to respect previous deletions
    if [[ -z "$(ls -A "${VOLUME_CONFIG_DIR}" 2>/dev/null)" ]]; then
        local config_source=""
        local discovery_dirs=(
            "${CONFIG_MOUNT}"
            "${ALTERNATIVE_CONFIG_MOUNT}"
        )
        info "Core: Volume is fresh. Searching for initial configuration..."
        ...
        elif [[ -f "${HOME}/.devcontainer.profile" ]] && [[ ! -L "${HOME}/.devcontainer.profile" ]]; then
            info "Core: Ingesting baked-in config to volume."
            cp "${HOME}/.devcontainer.profile" "${VOLUME_CONFIG_DIR}/config.json" || true
        fi
    fi
