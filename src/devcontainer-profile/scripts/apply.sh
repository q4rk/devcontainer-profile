...
calculate_hash() {
    ...
    if [[ -f "${USER_CONFIG_PATH}" ]]; then
        ...
        md5sum "${USER_CONFIG_PATH}" | awk '{print 
}'
...
main() {
    ...
    info "=== STARTING DEV CONTAINER PROFILE ENGINE (User: ${TARGET_USER}) ==="
    ...
    if [[ -z "$(ls -A "${VOLUME_CONFIG_DIR}" 2>/dev/null | grep -v "config.json")" ]]; then
